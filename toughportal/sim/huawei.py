...
class PortalSim(protocol.DatagramProtocol):
    ...
    def doAckLogout(self,req):
        ...
        resp2 = huawei.PortalV2.newMessage(
            huawei.NTF_LOGOUT,req.userIp,
            0,req.reqId,
            auth = req.auth,
            secret = self.secret
        )
    ...
    def datagramReceived(self, datagram, (host, port)):
        try:
            ...
            print huawei.hexdump(datagram,len(datagram))
            _packet = huawei.PortalV2(secret=self.secret,packet=datagram,source=(host, port))
            log.msg(":: Received portal packet from %s:%s: %s"%(host,port,repr(_packet)),level=logging.INFO)
            resp = None
            if _packet.type == huawei.REQ_CHALLENGE:
                resp = self.doAckChellenge(_packet)
            elif _packet.type == huawei.REQ_AUTH:
                resp = self.doAckAuth(_packet)
            elif _packet.type == huawei.REQ_LOGOUT:
                resp = self.doAckLogout(_packet)
            elif _packet.type == huawei.REQ_INFO:
                resp = self.doAckInfo(_packet)
            elif _packet.type == huawei.AFF_ACK_AUTH:
                pass
            elif _packet.type == huawei.ACK_NTF_LOGOUT:
                pass
            elif _packet.type == huawei.NTF_HEARTBEAT:
                pass
            else:
                log.msg("not support packet type: %s"%_packet.type)
            print
            if resp:
                print huawei.hexdump(str(resp),len(resp))
